...
class BackupsPage(ToolPage):
    ...
    def _draw_backup_entries(self):
        ...
        if not backups:
            ...
            empty.configure(background=self.root.style.colors.get("dark"), foreground=Style.DARK_GREY.value)
            empty.pack(pady=20)
            return

        type_colors = {"account": "#5865f2", "friends": "#4fee4c", "guilds": "#fee75c"}
        for backup in backups:
            card = RoundedFrame(self.backup_list_frame, radius=(8, 8, 8, 8), bootstyle="secondary.TFrame")
            card.pack(fill=ttk.X, pady=(0, 5))

            inner = RoundedFrame(card, radius=0, bootstyle="secondary.TFrame")
            inner.pack(fill=ttk.BOTH, padx=12, pady=8)

            name_frame = RoundedFrame(inner, radius=0, bootstyle="secondary.TFrame")
            ...
            type_label.configure(background=self.root.style.colors.get("secondary"))
            type_label.pack(side=ttk.LEFT, padx=(0, 5))

            # name_label = ttk.Label(name_frame, text=backup["name"], font=("Host Grotesk", 11, "bold"))
            # name_label.configure(background=self.root.style.colors.get("secondary"))
            ...
            info_label.configure(background=self.root.style.colors.get("secondary"), foreground=Style.LIGHT_GREY.value)
            info_label.grid(row=1, column=0, sticky=ttk.W)

            btn_frame = RoundedFrame(inner, radius=0, bootstyle="secondary.TFrame")
    ...
    @abc.abstractmethod
    def draw_content(self, inner_wrapper):
        create_wrapper = RoundedFrame(inner_wrapper, radius=10, bootstyle="dark.TFrame")
        ...
        btn_row = ttk.Frame(create_wrapper, style="dark.TFrame")
        ...
        backups_wrapper = RoundedFrame(inner_wrapper, radius=10, bootstyle="dark.TFrame")
        ...
        self.status_label.configure(background=self.root.style.colors.get("dark"), foreground=Style.LIGHT_GREY.value)
        self.status_label.grid(row=0, column=1, sticky=ttk.E, padx=(0, 10), pady=(10, 5))

        self.backup_list_frame = ScrolledFrame(backups_wrapper, bootstyle="dark.TFrame", autohide=True)
